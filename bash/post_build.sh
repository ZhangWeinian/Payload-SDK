...
TEMP_LIBS_OUTPUT_DIR="$5"
MAIN_OUTPUT_PATH="$6"
CONFIG_SOURCE_FILE="$7"
RUN_SCRIPT_SOURCE_FILE="$8"

FINAL_LIBS_OUTPUT_DIR="${VERSIONED_OUTPUT_PATH}/libs"
...
check_arg "${TEMP_LIBS_OUTPUT_DIR}" "临时库输出目录 (参数 5)"
...
echo "创建归档目录: ${VERSIONED_OUTPUT_PATH} 和 ${FINAL_LIBS_OUTPUT_DIR}"
mkdir -p "${VERSIONED_OUTPUT_PATH}"
mkdir -p "${FINAL_LIBS_OUTPUT_DIR}"

echo "复制可执行文件到归档目录: ${MAIN_TARGET_FILE} -> ${VERSIONED_OUTPUT_PATH}/${VERSIONED_EXE_NAME}"
cp -f "${MAIN_TARGET_FILE}" "${VERSIONED_OUTPUT_PATH}/${VERSIONED_EXE_NAME}"

echo "复制配置文件到归档目录: ${CONFIG_SOURCE_FILE} -> ${VERSIONED_OUTPUT_PATH}/config.yml"
cp -f "${CONFIG_SOURCE_FILE}" "${VERSIONED_OUTPUT_PATH}/config.yml"

echo "复制并设置 run.sh 到归档目录: ${RUN_SCRIPT_SOURCE_FILE} -> ${VERSIONED_OUTPUT_PATH}/run.sh"
cp -f "${RUN_SCRIPT_SOURCE_FILE}" "${VERSIONED_OUTPUT_PATH}/run.sh"
chmod +x "${VERSIONED_OUTPUT_PATH}/run.sh"

if [ -d "${SYSTEM_LIBS_SOURCE_DIR}" ] && [ -n "$(ls -A "${SYSTEM_LIBS_SOURCE_DIR}")" ]; then
    echo "复制系统库到归档目录: ${SYSTEM_LIBS_SOURCE_DIR} -> ${FINAL_LIBS_OUTPUT_DIR}"
    find "${SYSTEM_LIBS_SOURCE_DIR}" -maxdepth 1 -type f -exec cp -f {} "${FINAL_LIBS_OUTPUT_DIR}/" \;
else
    echo "警告: 系统库源目录 '${SYSTEM_LIBS_SOURCE_DIR}' 不存在或为空，跳过复制。"
fi

echo "复制临时文件到 build/bin/ ..."
...
if [ -d "${SYSTEM_LIBS_SOURCE_DIR}" ]; then
    mkdir -p "${TEMP_LIBS_OUTPUT_DIR}"
    find "${SYSTEM_LIBS_SOURCE_DIR}" -maxdepth 1 -type f -exec cp -f {} "${TEMP_LIBS_OUTPUT_DIR}/" \;
fi
