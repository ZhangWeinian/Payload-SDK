...
static T_DjiTestFlightControlVector3f	DjiTest_FlightControlVector3FSub(T_DjiTestFlightControlVector3f vectorA,
																		 T_DjiTestFlightControlVector3f vectorB);
...
static T_DjiReturnCode DjiTest_FlightControlJoystickCtrlAuthSwitchEventCallback(T_DjiFlightControllerJoystickCtrlAuthorityEventInfo eventData);
...
T_DjiReturnCode DjiTest_FlightControlRunSample(E_DjiTestFlightCtrlSampleSelect flightCtrlSampleSelect)
{
	...
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
		USER_LOG_WARN("Set RC lost action status disable failed,error code:0x%08llX, That means the aircraft will execute the RC lost Action "
					  "when RC is lost — RC-less flight not supported.",
					  returnCode);
		// go through
	}
...
T_DjiReturnCode DjiTest_FlightControlInit(void)
{
	...
	returnCode = DjiFcSubscription_SubscribeTopic(DJI_FC_SUBSCRIPTION_TOPIC_HEIGHT_FUSION, DJI_DATA_SUBSCRIPTION_TOPIC_10_HZ, NULL);
	...
	returnCode = DjiFcSubscription_SubscribeTopic(DJI_FC_SUBSCRIPTION_TOPIC_POSITION_FUSED, DJI_DATA_SUBSCRIPTION_TOPIC_50_HZ, NULL);
	...
	returnCode = DjiFcSubscription_SubscribeTopic(DJI_FC_SUBSCRIPTION_TOPIC_ALTITUDE_FUSED, DJI_DATA_SUBSCRIPTION_TOPIC_50_HZ, NULL);
	...
	returnCode = DjiFcSubscription_SubscribeTopic(DJI_FC_SUBSCRIPTION_TOPIC_ALTITUDE_OF_HOMEPOINT, DJI_DATA_SUBSCRIPTION_TOPIC_1_HZ, NULL);
	...
	returnCode = DjiFlightController_RegJoystickCtrlAuthorityEventCallback(DjiTest_FlightControlJoystickCtrlAuthSwitchEventCallback);
...
T_DjiReturnCode DjiTest_FlightControlDeInit(void)
{
	...
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
		USER_LOG_ERROR("Deinit flight controller module failed, error code:0x%08llX", returnCode);
		return returnCode;
	}
	...
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
		USER_LOG_ERROR("Deinit data subscription module failed, error code:0x%08llX", returnCode);
		return returnCode;
	}
...
void DjiTest_FlightControlPositionControlSample()
{
	...
	DjiTest_WidgetLogAppend("--> Step 4: Move to north:6(m), east:0(m), up:-3(m) , yaw:-30(degree) from current point");
...
void DjiTest_FlightControlVelocityControlSample()
{
	...
	USER_LOG_INFO("--> Step 3: Move with north:0(m/s), east:0(m/s), up:5(m/s), yaw:0(deg/s) from current point for 2s!");
	DjiTest_WidgetLogAppend("--> Step 3: Move with north:0(m/s), east:0(m/s), up:5(m/s), yaw:0(deg/s) from current point for 2s!");
	...
	USER_LOG_INFO("--> Step 5: Move with north:-1.5(m/s), east:2(m/s), up:0(m/s), yaw:20(deg/s) from current point for 2s!");
	DjiTest_WidgetLogAppend("--> Step 5: Move with north:-1.5(m/s), east:2(m/s), up:0(m/s), yaw:20(deg/s) from current point for 2s!");
	...
	USER_LOG_INFO("--> Step 7: Move with north:3(m/s), east:0(m/s), up:0(m/s), yaw:0(deg/s) from current point for 2.5s!");
	DjiTest_WidgetLogAppend("--> Step 7: Move with north:3(m/s), east:0(m/s), up:0(m/s), yaw:0(deg/s) from current point for 2.5s!");
	...
	USER_LOG_INFO("--> Step 9: Move with north:-1.6(m/s), east:-2(m/s), up:0(m/s), yaw:0(deg/s) from current point for 2.2s!");
	DjiTest_WidgetLogAppend("--> Step 9: Move with north:-1.6(m/s), east:-2(m/s), up:0(m/s), yaw:0(deg/s) from current point for 2.2s!");
...
void DjiTest_FlightControlSetGetParamSample()
{
	...
	if (aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M300_RTK || aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M30 ||
		aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M30T || aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M350_RTK)
	{
	...
	returnCode = DjiFlightController_SetUpwardsVisualObstacleAvoidanceEnableStatus(DJI_FLIGHT_CONTROLLER_ENABLE_OBSTACLE_AVOIDANCE);
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
	...
	returnCode = DjiFlightController_GetUpwardsVisualObstacleAvoidanceEnableStatus(&upwardsVisualObstacleAvoidanceStatus);
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
	...
	if (aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M300_RTK || aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M30 ||
		aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M30T || aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M350_RTK)
	{
		returnCode = DjiFlightController_SetUpwardsRadarObstacleAvoidanceEnableStatus(DJI_FLIGHT_CONTROLLER_ENABLE_OBSTACLE_AVOIDANCE);
		if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
		{
	...
	returnCode = DjiFlightController_SetDownwardsVisualObstacleAvoidanceEnableStatus(DJI_FLIGHT_CONTROLLER_ENABLE_OBSTACLE_AVOIDANCE);
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
	...
	returnCode = DjiFlightController_GetDownwardsVisualObstacleAvoidanceEnableStatus(&downloadsVisualObstacleAvoidanceStatus);
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
...
static void DjiTest_FlightControlSlowRotateMotorSample(void)
{
	...
	USER_LOG_INFO("The ESC status is: %s motor(s) in rotate mode",
				  ...
				  : escStatus == DJI_FLIGHT_CONTROLLER_ALL_MOTOR_IN_SLOW_ROTATE_MODE  ? "all"
																					  : "(error)");
...
T_DjiFcSubscriptionFlightStatus DjiTest_FlightControlGetValueOfFlightStatus(void)
{
	...
	{
		USER_LOG_DEBUG("Timestamp: millisecond %u microsecond %u.", flightStatusTimestamp.millisecond, flightStatusTimestamp.microsecond);
...
T_DjiFcSubscriptionDisplaymode DjiTest_FlightControlGetValueOfDisplayMode(void)
{
	...
	{
		USER_LOG_DEBUG("Timestamp: millisecond %u microsecond %u.", displayModeTimestamp.millisecond, displayModeTimestamp.microsecond);
...
T_DjiFcSubscriptionQuaternion DjiTest_FlightControlGetValueOfQuaternion(void)
{
	...
	{
		USER_LOG_DEBUG("Timestamp: millisecond %u microsecond %u.", quaternionTimestamp.millisecond, quaternionTimestamp.microsecond);
...
T_DjiFcSubscriptionPositionFused DjiTest_FlightControlGetValueOfPositionFused(void)
{
	...
	{
		USER_LOG_DEBUG("Timestamp: millisecond %u microsecond %u.", positionFusedTimestamp.millisecond, positionFusedTimestamp.microsecond);
		USER_LOG_DEBUG("PositionFused: %f, %f,%f,%d.",
...
dji_f32_t DjiTest_FlightControlGetValueOfRelativeHeight(void)
{
	...
	{
		USER_LOG_DEBUG("Timestamp: millisecond %u microsecond %u.", relativeHeightTimestamp.millisecond, relativeHeightTimestamp.microsecond);
	}
...
bool DjiTest_FlightControlMotorStartedCheck(void)
{
	...
	while (DjiTest_FlightControlGetValueOfFlightStatus() != DJI_FC_SUBSCRIPTION_FLIGHT_STATUS_ON_GROUND &&
		   DjiTest_FlightControlGetValueOfDisplayMode() != DJI_FC_SUBSCRIPTION_DISPLAY_MODE_ENGINE_START && motorsNotStarted < timeoutCycles)
	{
		motorsNotStarted++;
...
bool DjiTest_FlightControlCheckActionStarted(E_DjiFcSubscriptionDisplayMode mode)
{
	...
	if (actionNotStarted == timeoutCycles)
	{
		USER_LOG_ERROR("%s start failed, now flight is in %s.",
					   ...
					   s_flightControlDisplayModeStr[DjiTest_FlightControlGetDisplayModeIndex(DjiTest_FlightControlGetValueOfDisplayMode())]
						   .displayModeStr);
		return false;
	}
	else
	{
		USER_LOG_INFO("Now flight is in %s.", s_flightControlDisplayModeStr[DjiTest_FlightControlGetDisplayModeIndex(mode)].displayModeStr);
		return true;
	}
}
	...
	else
	{
			   ...
			   DjiTest_FlightControlGetValueOfFlightStatus() == DJI_FC_SUBSCRIPTION_FLIGHT_STATUS_IN_AIR)
		{
			...
			if (DJI_AIRCRAFT_TYPE_M3E == aircraftInfoBaseInfo.aircraftType || DJI_AIRCRAFT_TYPE_M3T == aircraftInfoBaseInfo.aircraftType ||
				...
				DJI_AIRCRAFT_TYPE_M3TA == aircraftInfoBaseInfo.aircraftType || DJI_AIRCRAFT_TYPE_M4T == aircraftInfoBaseInfo.aircraftType ||
				DJI_AIRCRAFT_TYPE_M4TD == aircraftInfoBaseInfo.aircraftType || DJI_AIRCRAFT_TYPE_M4D == aircraftInfoBaseInfo.aircraftType ||
				DJI_AIRCRAFT_TYPE_M4E == aircraftInfoBaseInfo.aircraftType || DJI_AIRCRAFT_TYPE_M4TD == aircraftInfoBaseInfo.aircraftType ||
...
bool DjiTest_FlightControlMoveByPositionOffset(const T_DjiTestFlightControlVector3f offsetDesired,
											   float								yawDesiredInDeg,
											   float								posThresholdInM,
											   float								yawThresholdInDeg)
{
	...
	T_DjiFlightControllerJoystickMode joystickMode = {
		DJI_FLIGHT_CONTROLLER_HORIZONTAL_POSITION_CONTROL_MODE, DJI_FLIGHT_CONTROLLER_VERTICAL_POSITION_CONTROL_MODE,
		DJI_FLIGHT_CONTROLLER_YAW_ANGLE_CONTROL_MODE,			DJI_FLIGHT_CONTROLLER_HORIZONTAL_GROUND_COORDINATE,
	...
	while (elapsedTimeInMs < timeoutInMilSec)
	{
		...
		T_DjiTestFlightControlVector3f localOffset = DjiTest_FlightControlLocalOffsetFromGpsAndFusedHeightOffset(currentGPSPosition,
																												 originGPSPosition,
																												 currentHeight,
		...
		T_DjiFlightControllerJoystickCommand joystickCommand = { positionCommand.x,
																 positionCommand.y,
...
void DjiTest_FlightControlVelocityAndYawRateCtrl(const T_DjiTestFlightControlVector3f offsetDesired, float yawRate, uint32_t timeMs)
{
	...
	T_DjiFlightControllerJoystickMode joystickMode = {
		DJI_FLIGHT_CONTROLLER_HORIZONTAL_VELOCITY_CONTROL_MODE, DJI_FLIGHT_CONTROLLER_VERTICAL_VELOCITY_CONTROL_MODE,
		DJI_FLIGHT_CONTROLLER_YAW_ANGLE_RATE_CONTROL_MODE,		DJI_FLIGHT_CONTROLLER_HORIZONTAL_GROUND_COORDINATE,
		DJI_FLIGHT_CONTROLLER_STABLE_CONTROL_MODE_ENABLE,
	};
...
T_DjiReturnCode DjiTest_FlightControlJoystickCtrlAuthSwitchEventCallback(T_DjiFlightControllerJoystickCtrlAuthorityEventInfo eventData)
{
...
static T_DjiReturnCode DjiTest_FlightControlSetFtsTrigger(E_DjiMountPosition position, const char* desc)
{
	...
	if (esc_status.ESC[0].fts_status == DJI_FLIGHT_CONTROLLER_FTS_NOT_TRIGGERD &&
		esc_status.ESC[1].fts_status == DJI_FLIGHT_CONTROLLER_FTS_NOT_TRIGGERD)
	{
