...
T_DjiReturnCode DjiTest_WaypointV3RunSample(void)
{
	...
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
		USER_LOG_ERROR("获取文件当前路径错误，错误码: 0x%08llX", returnCode);
		goto out;
	}
	...
	if (kmzFile == NULL)
	{
		USER_LOG_ERROR("打开 KMZ 文件失败！请确认 KMZ 文件位于 /tmp/kmz/1.kmz");
		goto out;
	}
	...
	if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS)
	{
		USER_LOG_ERROR("上传 KMZ 文件失败，错误码: 0x%08X");
		goto close_file;
	}
...
static T_DjiReturnCode DjiTest_WaypointV3ActionStateCallback(T_DjiWaypointV3ActionState actionState)
{
	USER_LOG_INFO("Waypoint v3 action state: %d, current waypoint index: %d, wayLine id: %d, action group: %d, action id: %d",
				  actionState.state,
