...
{
	double JsonToKmzConverter::calculateDistance(const protocol::Waypoint& wp1, const protocol::Waypoint& wp2)
	{
		const double R { 6'371'000 };
		double		 lat1Rad { wp1.WD * M_PI / 180.0 };
		double		 lat2Rad { wp2.WD * M_PI / 180.0 };
		double		 deltaLatRad { (wp2.WD - wp1.WD) * M_PI / 180.0 };
		double		 deltaLonRad { (wp2.JD - wp1.JD) * M_PI / 180.0 };
		double a { sin(deltaLatRad / 2) * sin(deltaLatRad / 2) + cos(lat1Rad) * cos(lat2Rad) * sin(deltaLonRad / 2) * sin(deltaLonRad / 2) };
		double c { 2 * atan2(sqrt(a), sqrt(1 - a)) };
		return R * c;
	}

	double JsonToKmzConverter::calculateTotalDistance(const std::vector<protocol::Waypoint>& waypoints)
	{
	...
	double JsonToKmzConverter::calculateTotalDuration(const std::vector<protocol::Waypoint>& waypoints)
	{
	...
	double JsonToKmzConverter::calculateHeadingAngle(const protocol::Waypoint& from, const protocol::Waypoint& to)
	{
		double deltaLon { (to.JD - from.JD) * M_PI / 180.0 };
		double fromLatRad { from.WD * M_PI / 180.0 };
		double toLatRad { to.WD * M_PI / 180.0 };
		double y { sin(deltaLon) * cos(toLatRad) };
		double x { cos(fromLatRad) * sin(toLatRad) - sin(fromLatRad) * cos(toLatRad) * cos(deltaLon) };
		double bearing { atan2(y, x) * 180.0 / M_PI };
		return fmod(bearing + 360.0, 360.0);
	}

	std::string JsonToKmzConverter::generateWaylinesKml(const std::vector<protocol::Waypoint>& waypoints)
	{
		std::ostringstream kml {};
		double			   totalDistance { calculateTotalDistance(waypoints) };
		double			   totalDuration { calculateTotalDuration(waypoints) };
		double			   avgSpeed { waypoints.empty() ? 5.0 : waypoints[0].SD.value_or(5.0) };
	...
	std::string JsonToKmzConverter::generateTemplateKml()
	{
												   ...
												   const protocol::WaypointPayload&		  missionInfo)
	{
		...
		LOG_INFO("开始生成 KMZ 文件，任务 ID: {}, 包含 {} 个航点...", missionInfo.RWID.value_or("N/A"), waypoints.size());
		...
		if (!archive)
		{
			...
			LOG_ERROR("无法创建或打开 KMZ 文件 '{}': {}", kmzFilePath, zip_error_strerror(&ziperror));
		...
		LOG_INFO("成功创建 KMZ 文件: {}", kmzFilePath);
		return true;
	}
} // namespace plane::utils
