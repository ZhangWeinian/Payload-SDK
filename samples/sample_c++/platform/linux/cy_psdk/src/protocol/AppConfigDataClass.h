...
namespace plane::protocol
{
    ...
    {
        ::std::string                node_id { "" };  // 探测身份 (需与服务端 local-node-id 匹配才会回复)
        ::std::uint16_t              port { 30'906 }; // UDP 探测端口 (服务端 swarm.udp.port)
        ::std::vector<::std::string> targets {};      // 探测目标 (单 IP / 末段通配 .* / CIDR / 起止范围)
    };

    struct AppConfigData
    {
        ::std::string_view mqttUrl {};                    // MQTT 服务器地址 (可选; 缺省由 SwarmCatalog 服务发现提供)
        ::std::string_view planeSn {};                    // 飞机序列号 (预留)
        ::std::string_view planeCode {};                  // 设备标识 (可选; 未配置时使用 PSDK 飞控序列号)
        ::std::string      mqttClientId {};               // MQTT 客户端 ID（自动生成）
        bool               enableFullPSDK { false };      // 是否启用完整 PSDK 功能
        bool               enableTraceLogLevel { false }; // 是否启用跟踪日志级别
        ::std::uint8_t     psdkLogLevel { 3 };            // 设置 PSDK 日志级别
