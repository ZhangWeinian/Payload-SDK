...
namespace plane::utils
{
	...
	void Logger::init(_SPDLOG level::level_enum console_level) noexcept
	{
		if (this->logger_)
		{
			return;
	...
	void Logger::setLocalLogFileLevel(_SPDLOG level::level_enum level) noexcept
	{
		if (this->logger_)
		{
	...
	void Logger::PSDKLogRedirection(const _STD string& rawMessage)
	{
		if (!this->logger_)
		{
			return;
		...
		try
		{
			...
			static const _STD regex pattern(R"((?:\x1b\[[0-9;]*m)?\s*([\d\.]+)\s+([^\s]+)\s+\[(\w+)\]\s+([^:]+:\d+)\s+(.*?)(?:\x1b\[0m)?\s*$)");
	...
	void Logger::manageLogFiles(const _STD_FS path& logDir, const _STD_FS path& newLogFile, _STD size_t maxFilesCount)
	{
		// 创建或更新指向最新日志文件的符号链接
